...
const Whiteboard = (props: WithTranslation): JSX.Element => {
    ...
    // JWT-carried whiteboard config wins over config.js so a single Jitsi
    // deployment can serve multiple WaveBook envs: each env's backend mints
    // a JWT with its own apiKey/apiUrl/collabServerBaseUrl. Jibri's recorder
    // JWT is generated by the same backend, so recordings inherit the same
    // env-specific routing automatically — config.js stays as a fallback for
    // direct (non-marketplace) access.
    const jwtWB = jwtCtx?.whiteboard;
    const collabServerBaseUrl = jwtWB?.collabServerBaseUrl || whiteboard?.collabServerBaseUrl;
    const apiBaseUrl = jwtWB?.apiUrl || whiteboard?.apiUrl || collabServerBaseUrl;
    const apiKey = jwtWB?.apiKey || whiteboard?.apiKey;
